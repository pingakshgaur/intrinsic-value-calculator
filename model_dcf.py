...
def growth_from_history(series, fy):
    ...
    if len(ys) < 2:
        return config.DEFAULT_GROWTH, (
            "only one usable FCFF observation; "
            f"used default growth {config.DEFAULT_GROWTH:.2%}"
        )
...
def intrinsic_value(company_data: dict, fy: int) -> Result:
    try:
        d = company_data.get("years", {}).get(fy)
        if not d:
        ...
        if base <= 0:
            return Result.bad(
                R.NOT_MEANINGFUL,
                f"FCFF for FY{fy} is negative ({config.CURRENCY_SYMBOL}{base:,.0f}); "
                f"a going-concern DCF cannot be run off a negative base cash flow "
                f"without an explicit turnaround forecast",
            )

        series = {y: _fcff_quiet(v, y) for y, v in company_data["years"].items()}
        ...
        if equity <= 0:
            return Result.bad(
                R.NEG_EQUITY,
                f"enterprise value {config.CURRENCY_SYMBOL}{ev:,.0f} is below net debt "
                f"{config.CURRENCY_SYMBOL}{net_debt:,.0f}; equity value is nil under "
                f"these assumptions (WACC {r:.2%}, growth {g:.2%})",
            )

        val = num(equity / shares)
        if val is None:
            return Result.bad(R.CALC_ERROR, "final per-share value was NaN/Inf")
        return Result.good(val)
